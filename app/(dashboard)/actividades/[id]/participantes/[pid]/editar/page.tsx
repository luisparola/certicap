...
export default function EditarParticipantePage() {
  ...
  useEffect(() => {
    fetch(`/api/actividades/${params.id}/participantes/${params.pid}`)
      .then((r) => r.json())
      .then((d) => {
        setTipoActividad(d.actividad?.tipo_certificado ?? "")
        setForm({
          nombre: d.nombre ?? "",
          rut: d.rut ?? "",
          nota_teoria: d.nota_teoria != null ? String(d.nota_teoria) : "",
          nota_practica: d.nota_practica != null ? String(d.nota_practica) : "",
          asistencia_pct: d.asistencia_pct != null ? String(d.asistencia_pct) : "",
          nro_registro: d.nro_registro ?? "",
          estado: d.estado ?? "PENDIENTE",
          marca_equipo: d.marca_equipo ?? "",
          modelo_equipo: d.modelo_equipo ?? "",
          capacidad_equipo: d.capacidad_equipo ?? "",
          senales: d.senales ?? "",
          espesor_diametro: d.espesor_diametro ?? "",
          aplicacion_soldadura: d.aplicacion_soldadura ?? "",
          observaciones: d.observaciones ?? "",
        })
      })
      .catch(() => toast({ variant: "destructive", title: "Error", description: "No se pudo cargar el participante." }))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/actividades/${params.id}/participantes/${params.pid}`, {
        method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form),
      })
      if (!res.ok) throw new Error("Error")
      toast({ title: "Participante actualizado", description: "Los cambios han sido guardados." })
      router.push(`/actividades/${params.id}`)
    } catch {
      toast({ variant: "destructive", title: "Error", description: "No se pudo actualizar el participante." })
  ...
  if (fetching) return <div className="flex justify-center py-20"><Loader2 className="h-8 w-8 animate-spin text-[#E8541A]" /></div>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
      <div className="flex items-center gap-4">
        ...
        <div><h1 className="text-2xl font-bold text-white">Editar Participante</h1><p className="text-gray-400 mt-1">Modifica los datos del participante</p></div>
      </div>
      <form onSubmit={handleSubmit}>
        <Card className="glass-card border-white/10">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="space-y-2"><Label className="text-gray-300">Nota Teoría</Label><Input type="number" step="0.1" min="1" max="7" value={form.nota_teoria} onChange={(e) => handleChange("nota_teoria", e.target.value)} className="bg-[#0F0F0F] border-white/10 text-white" /></div>
              <div className="space-y-2"><Label className="text-gray-300">Nota Práctica</Label><Input type="number" step="0.1" min="1" max="7" value={form.nota_practica} onChange={(e) => handleChange("nota_practica", e.target.value)} className="bg-[#0F0F0F] border-white/10 text-white" /></div>
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <Button type="submit" className="bg-[#E8541A] hover:bg-[#E8541A]/90" disabled={loading}>{loading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Guardando...</> : <><Save className="h-4 w-4 mr-2" />Guardar Cambios</>}</Button>
        </div>
      </form>
    </div>
  )
