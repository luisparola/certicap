        ...
        <Card className="glass-card border-white/10">
          <CardContent className="p-4 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div><Label className="text-xs text-gray-400">Año</Label><Select value={filters.anio} onValueChange={(v) => setFilters(f => ({...f, anio: v === "TODOS" ? "" : v}))}><SelectTrigger className={inputCls}><SelectValue placeholder="Todos" /></SelectTrigger><SelectContent className="bg-[#1A1A1A] border-white/10"><SelectItem value="TODOS">Todos</SelectItem>{years.map(y => <SelectItem key={y} value={y}>{y}</SelectItem>)}</SelectContent></Select></div>
